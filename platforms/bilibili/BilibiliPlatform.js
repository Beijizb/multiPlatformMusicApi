...
class BilibiliPlatform extends BasePlatform {
  ...
  async initialize() {
    ...
    try {
      ...
      this.logger.error('Bilibili platform initialization failed', error)
      throw error
    }
  }
  ...
  async callModule(route, req) {
    ...
    try {
      ...
      // 绑定辅助方法到 handler
      if (moduleConfig.parseDuration) {
        moduleConfig.handler = moduleConfig.handler.bind(moduleConfig)
      }
      if (moduleConfig.getAudioQuality) {
        moduleConfig.handler = moduleConfig.handler.bind(moduleConfig)
      }
